<template>
    <div>
        <div :class="{ 'hidden': isClosed }"
            class="fixed z-50 w-full h-full top-0 left-0 bg-white rounded-md px-8 flex justify-center items-center py-6 space-y-5 drop-shadow-lg">
            <div class="max-w-md">
              <h1 class="text-2xl ">{{ review ? "Edit review" : "Write a review" }}</h1>
              ...
              <div class="flex justify-end gap-4">
                ...
                <button @click.stop="onCancelButtonClicked"
                        class="px-5 py-2 bg-gray-500 hover:bg-gray-700 text-white cursor-pointer rounded-sm">
                  Cancel</button>
                <button @click.stop="onConfirmButtonClicked"
                        class="px-7 py-2 bg-blue-500 hover:bg-blue-700 text-white cursor-pointer rounded-sm">
                  {{ review ? "Edit Review" : "Post Review" }}</button>
              </div>
            </div>
        </div>
    </div>
